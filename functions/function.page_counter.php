...
function counter_get($art_id,$clang_id) {
   ...
   $date=strtotime($date);         // Wandlung in UNIX-Timestamp (Redaxo 4)
        ###echo "<div>$art_id, ".date("d.n.Y",$date)."</div>\n";
   #
   ...
   if(count($slice)>0):
     ...
     $date=strtotime($date);       // Wandlung in UNIX-Timestamp (Redaxo 4)
        ###echo "<div>$art_id, $slice[id], ".date("d.n.Y",$date)."</div>\n";
     endif;
   #
   ...
   if($diff-$days>0.5) $days=$days+1;
      ###echo "<div>$art_id, $slice[id], ".date("d.n.Y",$date).", $days Tage</div>\n";
   #
...
function counter_collect() {
   ...
   for($i=1;$i<=count($arts);$i=$i+1):
      ...
      $anz=counter_get($art_id,$clang_id);
           ###echo "<div>$i) $art_url, $art_count, $art_since</div>\n";
      $count[$i]=$anz[count];
...
function counter_out() {
   ...
   #
   echo "<h3 align=\"center\">Aufrufz&auml;hler f&uuml;r ausgew&auml;hlte Seiten</h3>\n";
   ...
   for($i=1;$i<=count($arts);$i=$i+1):
      ...
      echo "    <tr><td style=\"white-space:nowrap;\">$str:</td>\n".
